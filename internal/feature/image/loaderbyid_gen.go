...
func (l *LoaderByID) Load(key LoaderByIDKey) (*Image, error) {
...
func (l *LoaderByID) LoadThunk(key LoaderByIDKey) func() (*Image, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*Image, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *LoaderByID) LoadAll(keys []LoaderByIDKey) ([]*Image, []error) {
	results := make([]func() (*Image, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	images := make([]*Image, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *LoaderByID) LoadAllThunk(keys []LoaderByIDKey) func() ([]*Image, []error) {
	results := make([]func() (*Image, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *LoaderByID) Prime(key LoaderByIDKey, value *Image) bool {
	l.mu.Lock()
	var found bool
...
func (l *LoaderByID) unsafeSet(key LoaderByIDKey, value *Image) {
	if l.cache == nil {
		l.cache = map[LoaderByIDKey]*Image{}
	}
	l.cache[key] = value
}
